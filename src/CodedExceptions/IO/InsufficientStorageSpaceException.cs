...
public class InsufficientStorageSpaceException : System.IO.IOException
{
	...
	public InsufficientStorageSpaceException()
		: base(CreateMessage(null, null)) => StoragePath = null;
	...
	public override string ToString()
	{
		...
		{
			customText = CompositeFormatCache.Default.Format(nameof(SR.Global_FileName), StoragePath);
		}
	...
	protected InsufficientStorageSpaceException(SerializationInfo info, StreamingContext context)
		: base(info, context) => StoragePath = info.GetString(nameof(StoragePath));
	...
	private static string CreateMessage(string? message, string? fileName) => message ?? (fileName == null
			...
			: CompositeFormatCache.Default.Format(nameof(SR.InsufficientStorageSpaceException_MessagePath), fileName));
}
