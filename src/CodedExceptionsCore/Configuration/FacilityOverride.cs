	...
	internal class FacilityOverride : IEquatable<AssemblyName>
	{
		...
		public FacilityOverride(AssemblyName assemblyName, int identifier)
		{
			if (assemblyName == null)
				throw new CodedArgumentNullException(Errors.CreateHResult(ErrorCodes.FacilityOverride_ctor_AssemblyNull), nameof(assemblyName));
			if (identifier < 0 || identifier > 2047)
				throw new CodedArgumentOutOfRangeException(Errors.CreateHResult(ErrorCodes.FacilityOverride_ctor_FacilityOutOfRange), nameof(identifier), Properties.Resources.Global_FacilityId_OutOfRange);
		...
		public FacilityOverride(string assemblyName, int identifier)
		{
			if (string.IsNullOrWhiteSpace(assemblyName))
				throw new CodedArgumentNullException(Errors.CreateHResult(ErrorCodes.FacilityOverride_ctor_AssemblyStringNull), nameof(assemblyName));
			if (identifier < 0 || identifier > 2047)
				throw new CodedArgumentOutOfRangeException(Errors.CreateHResult(ErrorCodes.FacilityOverride_ctor_FacilityOutOfRange), nameof(identifier), Properties.Resources.Global_FacilityId_OutOfRange);
